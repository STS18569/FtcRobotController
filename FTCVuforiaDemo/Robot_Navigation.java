...
{
    // Constants
    private static final double  FOUND_THRESHOLD =  0.7;      //
    // private static final int     MAX_TARGETS     =    2;      // Was 4 in 2016-2017 - jjh 10/30/2020
     ...
     */
    public void initVuforiaOrig(LinearOpMode opMode, Robot_OmniDrive robot) {
        myOpMode = opMode;
        myRobot = robot;
    }
    ...
    public void initVuforia(LinearOpMode opMode, Robot_OmniDrive robot) {
        ...
        initTfod();
        targets = tfod.getUpdatedRecognitions();
        ...
        /*
        for (VuforiaTrackable trackable : tfod)
     ...
     */
    public boolean targetIsVisibleDisabled(int targetId) {
        Recognition target = targets.get(targetId);

        // if we have a target, look for an updated robot position
        if (target != null && (target.getConfidence() > FOUND_THRESHOLD)) {
            targetFound = true;
